...
export default function AdminProveedores() {
  ...
  const handleGuardar = async () => {
    ...
    try {
      if (editId) {
        await adminAPI.actualizarProveedor(editId, fd);
        toast.success('Proveedor actualizado');
      } else {
        await adminAPI.crearProveedor(fd);
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3, px: 1 }}>
          Gestiona las empresas y marcas que surten tus productos.
        </Typography>
          ...
          {proveedores.map(p => (
            <Grid item xs={12} sm={6} md={4} key={p.id}>
              <Card sx={{ borderRadius: 3, overflow: 'hidden', height: '100%',
                          ...
                          '&:hover': { transform: 'translateY(-3px)', boxShadow: '0 6px 20px rgba(194,24,91,0.15)' } }}>

                {/* Logo o placeholder */}
                {p.logoPath ? (
                  <CardMedia component="img" height="140"
                    image={`${BASE}${p.logoPath}`} alt={p.nombre}
                    sx={{ objectFit: 'contain', bgcolor: '#fafafa', p: 2 }} />
                ) : (
                  <Box sx={{ height: 140, bgcolor: '#fce4ec', display: 'flex',
                             alignItems: 'center', justifyContent: 'center' }}>
                    <Avatar sx={{ width: 72, height: 72, bgcolor: '#c2185b', fontSize: 28 }}>
                ...
                <CardContent sx={{ p: 2.5 }}>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                    ...
                    <Chip label={p.activo ? 'Activo' : 'Inactivo'}
                      color={p.activo ? 'success' : 'default'} size="small" />
                  </Box>

                  {/* Empresa */}
                  {p.empresa && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
                      <Business fontSize="small" sx={{ color: '#c2185b', fontSize: 16 }} />
                      <Typography variant="body2" color="text.secondary">{p.empresa}</Typography>
                    </Box>
                  )}
                  ...
                  {p.telefono && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
                      <Phone fontSize="small" sx={{ color: '#c2185b', fontSize: 16 }} />
                  ...
                  {p.correo && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
                      <Email fontSize="small" sx={{ color: '#c2185b', fontSize: 16 }} />
                      <Typography variant="body2" color="text.secondary" noWrap>{p.correo}</Typography>
                    </Box>
                  )}
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
                    <Button size="small" variant="outlined" startIcon={<Edit />}
                      sx={{ flex: 1 }} onClick={() => handleOpen(p)}>
                      Editar
                    </Button>
                    <Tooltip title="Eliminar">
                      <IconButton size="small" color="error" onClick={() => handleEliminar(p.id)}>
        ...
        <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
          ...
          <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
            ...
            <TextField label="Empresa / Marca" value={form.empresa}
              onChange={e => setForm({ ...form, empresa: e.target.value })} fullWidth />

            <TextField label="Teléfono" value={form.telefono}
              onChange={e => setForm({ ...form, telefono: e.target.value })} fullWidth />

            <TextField label="Correo" value={form.correo}
              onChange={e => setForm({ ...form, correo: e.target.value })} fullWidth />
            ...
            {logo && (
              <Typography variant="caption" color="success.main">✅ {logo.name}</Typography>
            )}
